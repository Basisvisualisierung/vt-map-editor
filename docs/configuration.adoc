...
== Template
The configuration file is a regular JSON file. Here is a template for the configuration:

.config.json
[source,json]
----
include::../src/assets/config/config_template.json[]
----
...
URL of the backend service _vt-map-service_.
...
Maximium zoom level of the map (\<= 22).
...
Show / hide the toggle for the rounting function in the functions user interface. The routing function is only embedded in published maps.
...
=== basemaps
List of avaliable basemaps in the application. Each item has the following attributes:
